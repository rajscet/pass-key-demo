...
const RP_ID_HOST = (RP_ID || "")
  .replace(/^https?:\/\//, "")
  .replace(/\/.*/, "");
const CREDENTIALS_TABLE =
  process.env.CREDENTIALS_TABLE || "passkey_credentials";

function uuidToBase64Url(uuid) {
  const hex = String(uuid).replace(/-/g, "").toLowerCase();
  if (hex.length !== 32)
    return Buffer.from(String(uuid), "utf8").toString("base64url");
...
function normalizePublicKey(pk) {
  // Shape logging
  try {
    const kind = Buffer.isBuffer(pk)
      ? "Buffer"
      : typeof pk === "string"
      ? "string"
      : pk && typeof pk === "object" && pk.byteLength
      ? "TypedArray"
      : pk && typeof pk === "object" && Array.isArray(pk.data)
      ? "BufferJSON"
      : typeof pk;
    const sample =
      typeof pk === "string"
        ...
        : pk && pk.byteLength
        ? Buffer.from(pk).slice(0, 8).toString("hex")
        : "";
  ...
  if (typeof pk === "string") {
    ...
    try {
      return Buffer.from(pk, "base64url");
    } catch {}
    try {
...
export async function startRegistration(user) {
  ...
  if (chErr)
    throw new Error(
      `Failed to save challenge (registration): ${chErr.message}`
    );
...
export async function finishRegistration(user, credential) {
  console.log("finishRegistration called for userId", user.id);
  const { data: challengeRow } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  console.log("challengeRow", challengeRow);
  if (!challengeRow?.challenge) {
    throw new Error(
      "No saved registration challenge found; start registration again."
    );
  }
...
export async function startAuthentication(user) {
  ...
  if (chErr)
    throw new Error(
      `Failed to save challenge (authentication): ${chErr.message}`
    );
...
export async function finishAuthentication(user, assertion) {
  ...
  try {
    ...
    if (chErr)
      return {
        verified: false,
        reason: `challenge fetch failed: ${chErr.message}`,
      };
    if (!challengeRow?.challenge)
      return { verified: false, reason: "no saved challenge" };
    if (!cred)
      return { verified: false, reason: "no authenticator for credential id" };
    ...
    try {
      ...
      console.log(
        "🔎 clientDataJSON.challenge(base64url)=",
    ...
    console.log(
      "🔎 expectedRPID   =",
      (process.env.RP_ID || "").replace(/^https?:\/\//, "").replace(/\/.*/, "")
    );
